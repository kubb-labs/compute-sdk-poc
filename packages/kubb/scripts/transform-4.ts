 ...
 */
export const removeBrokenSchemas = ({
  spec,
}: {
  spec: unknown;
}) => {
  walkSchemas({
    ...
    visitor: (nodeInfo) => {
      if (
        ...
        'allOf' in nodeInfo.node &&
        Array.isArray(nodeInfo.node.allOf)
      ) {
        for (let i = 0; i < nodeInfo.node.allOf.length; i++) {
          if (isInvalidSchema(nodeInfo.node.allOf[i])) {
            nodeInfo.node.allOf.splice(i, 1);
          }
        }
      }
    },
...
function isInvalidSchema(schema: Record<string, unknown>) {
