...
function getPostChildAccountTokenUrl(euuId: PostChildAccountTokenPathParams["euuId"]) {
  const res = { method: 'POST', url: `/account/child-accounts/${euuId}/token` as const }  
  return res
}
 ...
 * {@link /account/child-accounts/:euuId/token}
 */
export async function postChildAccountToken(euuId: PostChildAccountTokenPathParams["euuId"], config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const res = await request<PostChildAccountTokenMutationResponse, ResponseErrorConfig<Error>, unknown>({ method : "POST", url : getPostChildAccountTokenUrl(euuId).url.toString(), ... requestConfig })  
  return res.data
}