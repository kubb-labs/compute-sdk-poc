...
function getPostLinodeInterfaceUrl(linodeId: PostLinodeInterfacePathParams["linodeId"]) {
  const res = { method: 'POST', url: `/linode/instances/${linodeId}/interfaces` as const }  
  return res
}
 ...
 */
export async function postLinodeInterface(linodeId: PostLinodeInterfacePathParams["linodeId"], data?: PostLinodeInterfaceMutationRequest, config: Partial<RequestConfig<PostLinodeInterfaceMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostLinodeInterfaceMutationResponse, ResponseErrorConfig<Error>, PostLinodeInterfaceMutationRequest>({ method : "POST", url : getPostLinodeInterfaceUrl(linodeId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}