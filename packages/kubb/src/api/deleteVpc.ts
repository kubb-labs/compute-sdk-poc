...
function getDeleteVpcUrl(vpcId: DeleteVpcPathParams["vpcId"]) {
  const res = { method: 'DELETE', url: `/vpcs/${vpcId}` as const }  
  return res
}
 ...
 * {@link /vpcs/:vpcId}
 */
export async function deleteVpc(vpcId: DeleteVpcPathParams["vpcId"], config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const res = await request<DeleteVpcMutationResponse, ResponseErrorConfig<Error>, unknown>({ method : "DELETE", url : getDeleteVpcUrl(vpcId).url.toString(), ... requestConfig })  
  return res.data
}