...
import type { PutLongviewPlanMutationRequest, PutLongviewPlanMutationResponse } from "../types/PutLongviewPlan.ts";
import { fetch } from "../.kubb/fetch.ts";

function getPutLongviewPlanUrl() {
  const res = { method: 'PUT', url: `/longview/plan` as const }  
  return res
}
 ...
 * {@link /longview/plan}
 */
export async function putLongviewPlan(data?: PutLongviewPlanMutationRequest, config: Partial<RequestConfig<PutLongviewPlanMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PutLongviewPlanMutationResponse, ResponseErrorConfig<Error>, PutLongviewPlanMutationRequest>({ method : "PUT", url : getPutLongviewPlanUrl().url.toString(), data : requestData, ... requestConfig })  
  return res.data
}