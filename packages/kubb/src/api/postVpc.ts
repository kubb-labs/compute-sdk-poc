...
import type { PostVpcMutationRequest, PostVpcMutationResponse } from "../types/PostVpc.ts";
import { fetch } from "../.kubb/fetch.ts";

function getPostVpcUrl() {
  const res = { method: 'POST', url: `/vpcs` as const }  
  return res
}
 ...
 * {@link /vpcs}
 */
export async function postVpc(data?: PostVpcMutationRequest, config: Partial<RequestConfig<PostVpcMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostVpcMutationResponse, ResponseErrorConfig<Error>, PostVpcMutationRequest>({ method : "POST", url : getPostVpcUrl().url.toString(), data : requestData, ... requestConfig })  
  return res.data
}