...
function getPostNodeBalancerConfigUrl(nodeBalancerId: PostNodeBalancerConfigPathParams["nodeBalancerId"]) {
  const res = { method: 'POST', url: `/nodebalancers/${nodeBalancerId}/configs` as const }  
  return res
}
 ...
 */
export async function postNodeBalancerConfig(nodeBalancerId: PostNodeBalancerConfigPathParams["nodeBalancerId"], data?: PostNodeBalancerConfigMutationRequest, config: Partial<RequestConfig<PostNodeBalancerConfigMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostNodeBalancerConfigMutationResponse, ResponseErrorConfig<Error>, PostNodeBalancerConfigMutationRequest>({ method : "POST", url : getPostNodeBalancerConfigUrl(nodeBalancerId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}