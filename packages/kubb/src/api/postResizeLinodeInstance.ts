...
function getPostResizeLinodeInstanceUrl(linodeId: PostResizeLinodeInstancePathParams["linodeId"]) {
  const res = { method: 'POST', url: `/linode/instances/${linodeId}/resize` as const }  
  return res
}
 ...
 * {@link /linode/instances/:linodeId/resize}
 */
export async function postResizeLinodeInstance(linodeId: PostResizeLinodeInstancePathParams["linodeId"], data: PostResizeLinodeInstanceMutationRequest, config: Partial<RequestConfig<PostResizeLinodeInstanceMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostResizeLinodeInstanceMutationResponse, ResponseErrorConfig<Error>, PostResizeLinodeInstanceMutationRequest>({ method : "POST", url : getPostResizeLinodeInstanceUrl(linodeId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}