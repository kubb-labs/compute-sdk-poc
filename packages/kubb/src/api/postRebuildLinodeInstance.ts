...
function getPostRebuildLinodeInstanceUrl(linodeId: PostRebuildLinodeInstancePathParams["linodeId"]) {
  const res = { method: 'POST', url: `/linode/instances/${linodeId}/rebuild` as const }  
  return res
}
 ...
 * {@link /linode/instances/:linodeId/rebuild}
 */
export async function postRebuildLinodeInstance(linodeId: PostRebuildLinodeInstancePathParams["linodeId"], data: PostRebuildLinodeInstanceMutationRequest, config: Partial<RequestConfig<PostRebuildLinodeInstanceMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostRebuildLinodeInstanceMutationResponse, ResponseErrorConfig<Error>, PostRebuildLinodeInstanceMutationRequest>({ method : "POST", url : getPostRebuildLinodeInstanceUrl(linodeId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}