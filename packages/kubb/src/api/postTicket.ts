...
import type { PostTicketMutationRequest, PostTicketMutationResponse } from "../types/PostTicket.ts";
import { fetch } from "../.kubb/fetch.ts";

function getPostTicketUrl() {
  const res = { method: 'POST', url: `/support/tickets` as const }  
  return res
}
 ...
 */
export async function postTicket(data: PostTicketMutationRequest, config: Partial<RequestConfig<PostTicketMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostTicketMutationResponse, ResponseErrorConfig<Error>, PostTicketMutationRequest>({ method : "POST", url : getPostTicketUrl().url.toString(), data : requestData, ... requestConfig })  
  return res.data
}