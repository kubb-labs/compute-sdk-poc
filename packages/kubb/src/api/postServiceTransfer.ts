...
import type { PostServiceTransferMutationRequest, PostServiceTransferMutationResponse } from "../types/PostServiceTransfer.ts";
import { fetch } from "../.kubb/fetch.ts";

function getPostServiceTransferUrl() {
  const res = { method: 'POST', url: `/account/service-transfers` as const }  
  return res
}
 ...
 */
export async function postServiceTransfer(data: PostServiceTransferMutationRequest, config: Partial<RequestConfig<PostServiceTransferMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostServiceTransferMutationResponse, ResponseErrorConfig<Error>, PostServiceTransferMutationRequest>({ method : "POST", url : getPostServiceTransferUrl().url.toString(), data : requestData, ... requestConfig })  
  return res.data
}