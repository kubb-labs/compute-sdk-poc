...
function getPostAcceptServiceTransferUrl(token: PostAcceptServiceTransferPathParams["token"]) {
  const res = { method: 'POST', url: `/account/service-transfers/${token}/accept` as const }  
  return res
}
 ...
 * {@link /account/service-transfers/:token/accept}
 */
export async function postAcceptServiceTransfer(token: PostAcceptServiceTransferPathParams["token"], config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const res = await request<PostAcceptServiceTransferMutationResponse, ResponseErrorConfig<Error>, unknown>({ method : "POST", url : getPostAcceptServiceTransferUrl(token).url.toString(), ... requestConfig })  
  return res.data
}