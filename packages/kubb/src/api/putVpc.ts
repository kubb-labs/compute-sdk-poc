...
function getPutVpcUrl(vpcId: PutVpcPathParams["vpcId"]) {
  const res = { method: 'PUT', url: `/vpcs/${vpcId}` as const }  
  return res
}
 ...
 * {@link /vpcs/:vpcId}
 */
export async function putVpc(vpcId: PutVpcPathParams["vpcId"], data?: PutVpcMutationRequest, config: Partial<RequestConfig<PutVpcMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PutVpcMutationResponse, ResponseErrorConfig<Error>, PutVpcMutationRequest>({ method : "PUT", url : getPutVpcUrl(vpcId).url.toString(), data : requestData, ... requestConfig })  
  return res.data
}