...
import type { PostPlacementGroupMutationRequest, PostPlacementGroupMutationResponse } from "../types/PostPlacementGroup.ts";
import { fetch } from "../.kubb/fetch.ts";

function getPostPlacementGroupUrl() {
  const res = { method: 'POST', url: `/placement/groups` as const }  
  return res
}
 ...
 */
export async function postPlacementGroup(data: PostPlacementGroupMutationRequest, config: Partial<RequestConfig<PostPlacementGroupMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const requestData = data  
  
  const res = await request<PostPlacementGroupMutationResponse, ResponseErrorConfig<Error>, PostPlacementGroupMutationRequest>({ method : "POST", url : getPostPlacementGroupUrl().url.toString(), data : requestData, ... requestConfig })  
  return res.data
}