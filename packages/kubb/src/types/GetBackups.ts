/**
* Generated by Kubb (https://kubb.dev/).
* Do not edit manually.
*/


export const getBackupsPathParamsApiVersionEnum = {
    "v4": "v4",
    "v4beta": "v4beta"
} as const;

export type GetBackupsPathParamsApiVersionEnumKey = (typeof getBackupsPathParamsApiVersionEnum)[keyof typeof getBackupsPathParamsApiVersionEnum];

export interface GetBackupsPathParams {
    /**
     * @description __Enum__ Call either the `v4` URL, or `v4beta` for operations still in Beta.
     * @type string
    */
    apiVersion: GetBackupsPathParamsApiVersionEnumKey;
    /**
     * @description The ID of the Linode the backups belong to.
     * @type integer
    */
    linodeId: number;
}

export const disksFilesystemEnum2 = {
    "raw": "raw",
    "swap": "swap",
    "ext2": "ext2",
    "ext3": "ext3",
    "ext4": "ext4",
    "initrd": "initrd"
} as const;

export type DisksFilesystemEnum2Key = (typeof disksFilesystemEnum2)[keyof typeof disksFilesystemEnum2];

export const automaticStatusEnum = {
    "paused": "paused",
    "pending": "pending",
    "running": "running",
    "needsPostProcessing": "needsPostProcessing",
    "successful": "successful",
    "failed": "failed",
    "userAborted": "userAborted"
} as const;

export type AutomaticStatusEnumKey = (typeof automaticStatusEnum)[keyof typeof automaticStatusEnum];

export const disksFilesystemEnum3 = {
    "raw": "raw",
    "swap": "swap",
    "ext2": "ext2",
    "ext3": "ext3",
    "ext4": "ext4",
    "initrd": "initrd"
} as const;

export type DisksFilesystemEnum3Key = (typeof disksFilesystemEnum3)[keyof typeof disksFilesystemEnum3];

export const currentStatusEnum = {
    "paused": "paused",
    "pending": "pending",
    "running": "running",
    "needsPostProcessing": "needsPostProcessing",
    "successful": "successful",
    "failed": "failed",
    "userAborted": "userAborted"
} as const;

export type CurrentStatusEnumKey = (typeof currentStatusEnum)[keyof typeof currentStatusEnum];

export const currentTypeEnum = {
    "auto": "auto",
    "snapshot": "snapshot"
} as const;

export type CurrentTypeEnumKey = (typeof currentTypeEnum)[keyof typeof currentTypeEnum];

export const disksFilesystemEnum4 = {
    "raw": "raw",
    "swap": "swap",
    "ext2": "ext2",
    "ext3": "ext3",
    "ext4": "ext4",
    "initrd": "initrd"
} as const;

export type DisksFilesystemEnum4Key = (typeof disksFilesystemEnum4)[keyof typeof disksFilesystemEnum4];

export const inProgressStatusEnum = {
    "paused": "paused",
    "pending": "pending",
    "running": "running",
    "needsPostProcessing": "needsPostProcessing",
    "successful": "successful",
    "failed": "failed",
    "userAborted": "userAborted"
} as const;

export type InProgressStatusEnumKey = (typeof inProgressStatusEnum)[keyof typeof inProgressStatusEnum];

export const inProgressTypeEnum = {
    "auto": "auto",
    "snapshot": "snapshot"
} as const;

export type InProgressTypeEnumKey = (typeof inProgressTypeEnum)[keyof typeof inProgressTypeEnum];

/**
 * @description A collection of the specified Linode\'s available backups.
*/
export interface GetBackups200 {
    /**
     * @type array | undefined
    */
    automatic?: {
        /**
         * @description __Read-only__ Whether this Backup is available for restoration.\n\nBackups undergoing maintenance are not available for restoration.
         * @type boolean | undefined
        */
        readonly available?: boolean;
        /**
         * @description __Read-only__ A list of the labels of the Configuration profiles that are part of the Backup.
         * @type array | undefined
        */
        readonly configs?: string[];
        /**
         * @description __Read-only__ The date the Backup was taken.
         * @type string | undefined, date-time
        */
        readonly created?: string;
        /**
         * @description __Read-only__ A list of the disks that are part of the Backup.
         * @type array | undefined
        */
        readonly disks?: {
            /**
             * @description The disk\'s format or file system. A value of `raw` indicates no file system, just a raw binary stream. A value of `swap` indicates a Linux swap area. The values `ext3` or `ext4` represent these Linux journaling file systems. The value `ext2` is the deprecated ext2 Linux file system. Finally, `initrd` indicates the disk is formatted as an uncompressed initial RAM disk.\n\n> 📘\n>\n> The `ext2` file system doesn\'t properly support timestamps and will be removed from Linux support in the near future. Also, `initrd` is a legacy format that no longer applies to most use cases. As a best practice, use the other supported formats or file systems instead.
             * @type string | undefined
            */
            filesystem?: DisksFilesystemEnum2Key;
            /**
             * @type string | undefined
            */
            label?: string;
            /**
             * @type integer | undefined
            */
            size?: number;
        }[];
        /**
         * @description __Read-only__ The date the Backup completed.
         * @type string | undefined, date-time
        */
        readonly finished?: string;
        /**
         * @description __Read-only__ The unique ID of this Backup.
         * @type integer | undefined
        */
        readonly id?: number;
        /**
         * @description A label for Backups that are of type `snapshot`.
         * @type string
        */
        label?: string | null;
        /**
         * @description __Read-only__ The current state of a specific Backup.
         * @type string | undefined
        */
        readonly status?: AutomaticStatusEnumKey;
        /**
         * @type string | undefined
        */
        type?: string;
        /**
         * @description __Read-only__ The date the Backup was most recently updated.
         * @type string | undefined, date-time
        */
        readonly updated?: string;
    }[];
    /**
     * @type object | undefined
    */
    snapshot?: {
        /**
         * @description An object representing a Backup or snapshot for a Linode with Backup service enabled.
         * @type object | undefined
        */
        current?: {
            /**
             * @description __Read-only__ Whether this Backup is available for restoration.\n\nBackups undergoing maintenance are not available for restoration.
             * @type boolean | undefined
            */
            readonly available?: boolean;
            /**
             * @description __Read-only__ A list of the labels of the Configuration profiles that are part of the Backup.
             * @type array | undefined
            */
            readonly configs?: string[];
            /**
             * @description __Read-only__ The date the Backup was taken.
             * @type string | undefined, date-time
            */
            readonly created?: string;
            /**
             * @description __Read-only__ A list of the disks that are part of the Backup.
             * @type array | undefined
            */
            readonly disks?: {
                /**
                 * @description The disk\'s format or file system. A value of `raw` indicates no file system, just a raw binary stream. A value of `swap` indicates a Linux swap area. The values `ext3` or `ext4` represent these Linux journaling file systems. The value `ext2` is the deprecated ext2 Linux file system. Finally, `initrd` indicates the disk is formatted as an uncompressed initial RAM disk.\n\n> 📘\n>\n> The `ext2` file system doesn\'t properly support timestamps and will be removed from Linux support in the near future. Also, `initrd` is a legacy format that no longer applies to most use cases. As a best practice, use the other supported formats or file systems instead.
                 * @type string | undefined
                */
                filesystem?: DisksFilesystemEnum3Key;
                /**
                 * @type string | undefined
                */
                label?: string;
                /**
                 * @type integer | undefined
                */
                size?: number;
            }[];
            /**
             * @description __Read-only__ The date the Backup completed.
             * @type string | undefined, date-time
            */
            readonly finished?: string;
            /**
             * @description __Read-only__ The unique ID of this Backup.
             * @type integer | undefined
            */
            readonly id?: number;
            /**
             * @description A label for Backups that are of type `snapshot`.
             * @type string
            */
            label?: string | null;
            /**
             * @description __Read-only__ The current state of a specific Backup.
             * @type string | undefined
            */
            readonly status?: CurrentStatusEnumKey;
            /**
             * @description __Read-only__ This indicates whether the Backup is an automatic Backup or manual snapshot taken by the User at a specific point in time.
             * @type string | undefined
            */
            readonly type?: CurrentTypeEnumKey;
            /**
             * @description __Read-only__ The date the Backup was most recently updated.
             * @type string | undefined, date-time
            */
            readonly updated?: string;
        };
        /**
         * @description An object representing a Backup or snapshot for a Linode with Backup service enabled.
         * @type object | undefined
        */
        in_progress?: {
            /**
             * @description __Read-only__ Whether this Backup is available for restoration.\n\nBackups undergoing maintenance are not available for restoration.
             * @type boolean | undefined
            */
            readonly available?: boolean;
            /**
             * @description __Read-only__ A list of the labels of the Configuration profiles that are part of the Backup.
             * @type array | undefined
            */
            readonly configs?: string[];
            /**
             * @description __Read-only__ The date the Backup was taken.
             * @type string | undefined, date-time
            */
            readonly created?: string;
            /**
             * @description __Read-only__ A list of the disks that are part of the Backup.
             * @type array | undefined
            */
            readonly disks?: {
                /**
                 * @description The disk\'s format or file system. A value of `raw` indicates no file system, just a raw binary stream. A value of `swap` indicates a Linux swap area. The values `ext3` or `ext4` represent these Linux journaling file systems. The value `ext2` is the deprecated ext2 Linux file system. Finally, `initrd` indicates the disk is formatted as an uncompressed initial RAM disk.\n\n> 📘\n>\n> The `ext2` file system doesn\'t properly support timestamps and will be removed from Linux support in the near future. Also, `initrd` is a legacy format that no longer applies to most use cases. As a best practice, use the other supported formats or file systems instead.
                 * @type string | undefined
                */
                filesystem?: DisksFilesystemEnum4Key;
                /**
                 * @type string | undefined
                */
                label?: string;
                /**
                 * @type integer | undefined
                */
                size?: number;
            }[];
            /**
             * @description __Read-only__ The date the Backup completed.
             * @type string | undefined, date-time
            */
            readonly finished?: string;
            /**
             * @description __Read-only__ The unique ID of this Backup.
             * @type integer | undefined
            */
            readonly id?: number;
            /**
             * @description A label for Backups that are of type `snapshot`.
             * @type string
            */
            label?: string | null;
            /**
             * @description __Read-only__ The current state of a specific Backup.
             * @type string | undefined
            */
            readonly status?: InProgressStatusEnumKey;
            /**
             * @description __Read-only__ This indicates whether the Backup is an automatic Backup or manual snapshot taken by the User at a specific point in time.
             * @type string | undefined
            */
            readonly type?: InProgressTypeEnumKey;
            /**
             * @description __Read-only__ The date the Backup was most recently updated.
             * @type string | undefined, date-time
            */
            readonly updated?: string;
        };
    };
}

/**
 * @description See [Errors](https://techdocs.akamai.com/linode-api/reference/errors) for the range of possible error response codes.
*/
export interface GetBackupsError {
    /**
     * @type array | undefined
    */
    errors?: {
        /**
         * @description The field in the request that caused this error. This may be a path, separated by periods in the case of nested fields. In some cases this may come back as `null` if the error is not specific to any single element of the request.
         * @type string | undefined
        */
        field?: string;
        /**
         * @description What happened to cause this error. In most cases, this can be fixed immediately by changing the data you sent in the request, but in some cases you will be instructed to [Open a support ticket](https://techdocs.akamai.com/linode-api/reference/post-ticket) or perform some other action before you can complete the request successfully.
         * @type string | undefined
        */
        reason?: string;
    }[];
}

export type GetBackupsQueryResponse = GetBackups200;

export type GetBackupsQuery = {
    Response: GetBackups200;
    PathParams: GetBackupsPathParams;
    Errors: any;
};