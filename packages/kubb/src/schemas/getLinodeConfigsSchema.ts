...
export const getLinodeConfigs200Schema = z.object({
    ...
    "comments": z.nullable(z.string().describe("Optional field for arbitrary user comments on this configuration.")),
"devices": z.object({
    "sda": z.object({
    "disk_id": z.int().describe("The Disk ID, or `null` if a Volume is assigned to this slot."),
"volume_id": z.nullable(z.int().describe("The Volume ID, or `null` if a Disk is assigned to this slot."))
...
"helpers": z.object({
    "devtmpfs_automount": z.boolean().default(false).describe("Populates the `/dev` directory early during boot without `udev`.  Defaults to `false`."),
"distro": z.boolean().describe("Helps maintain correct `inittab` or `upstart` console device."),
"modules_dep": z.boolean().describe("Creates a modules dependency file for the kernel you run."),
"network": z.nullable(z.boolean().describe("Set to `true` to automatically configure static networking. The `network` option applies only to legacy configuration profile interfaces and does not apply to [Linode interfaces](https://techdocs.akamai.com/linode-api/reference/post-linode-interface).")),
"updatedb_disabled": z.boolean().describe("Set to `true` to disable the `updatedb` cron job to avoid disk thrashing.")
    }).describe("Helpers enabled when booting to this Linode configuration."),
"id": z.int().describe("__Read-only__ The ID of this Config."),
...
"kernel": z.string().default("linode/latest-64bit").describe("The ID of the kernel used to boot a Linode. Run the [List kernels](https://techdocs.akamai.com/linode-api/reference/get-kernels) operation to see all available kernels. Here are some commonly used kernels:\n\n- `linode/latest-64bit`. This is the default, our latest kernel at the time of an instance boot or reboot.\n\n- `linode/grub2`. The upstream distribution-supplied kernel that's installed on the primary disk, or a custom kernel if installed.\n\n- `linode/direct-disk`. The master boot record (MBR) of the primary disk or root device. Use this in place of a Linux kernel."),
"label": z.string().min(1).max(48).describe("__Filterable__ The name of the configuration for display in Akamai Cloud Manager."),
"memory_limit": z.int().describe("Defaults to the total RAM of the Linode."),
"root_device": z.string().regex(/a-z, A-Z, 0-9, \/, _, -/).describe("The root device to boot.\n\n> 📘\n\n- If you leave this empty or set an invalid value, the root device defaults to `/dev/sda`.\n\n- If you specify a device at the root device location and it's not mounted, the Linode won't boot until a device is mounted."),
"run_level": z.enum(["default", "single", "binbash"]).describe("Defines the state of your Linode after booting. Defaults to `default`."),
