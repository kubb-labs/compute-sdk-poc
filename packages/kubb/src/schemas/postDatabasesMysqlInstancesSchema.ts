 ...
 */
export const postDatabasesMysqlInstances200Schema = z.object({
...
"engine_config": z.object({
    "binlog_retention_period": z.int().min(600).max(86400).describe("The minimum amount of time in seconds to keep `binlog` entries before deletion. This may be extended for services that require `binlog` entries for longer than the default, for example if using the MySQL Debezium Kafka connector."),
"mysql": z.object({
    "connect_timeout": z.int().min(2).max(3600).describe("The number of seconds that the `mysqld` server waits for a connect packet before responding with bad handshake."),
"default_time_zone": z.string().regex(/^([-+][\\d:]*|[\\w/]*)$/).describe("Default server time zone as an offset from UTC (from -12:00 to +12:00), a time zone name, or `SYSTEM` to use the MySQL server default."),
"group_concat_max_len": z.int().min(4).max(18446744073709552000).describe("The maximum permitted result length in bytes for the `GROUP_CONCAT()` function."),
"information_schema_stats_expiry": z.int().min(900).max(31536000).describe("The time, in seconds, before cached statistics expire."),
"innodb_change_buffer_max_size": z.int().min(0).max(50).default(25).describe("Maximum size for the InnoDB change buffer, as a percentage of the total size of the buffer pool."),
"innodb_flush_neighbors": z.int().min(0).max(2).default(1).describe("Specifies whether flushing a page from the InnoDB buffer pool also flushes other dirty pages in the same extent: `0` - dirty pages in the same extent are not flushed, `1` - flush contiguous dirty pages in the same extent, `2` - flush dirty pages in the same extent."),
"innodb_ft_min_token_size": z.int().min(0).max(16).describe("Minimum length of words that are stored in an InnoDB `-1FULLTEXT` index. Changing this parameter will lead to a restart of the MySQL service."),
"innodb_ft_server_stopword_table": z.nullable(z.string().regex(/^.+\/.+$/).describe("This option is used to specify your own InnoDB `FULLTEXT` index `stopword` list for all InnoDB tables. Set to `null` for no value.")),
"innodb_lock_wait_timeout": z.int().min(1).max(3600).default(120).describe("The length of time in seconds an InnoDB transaction waits for a row lock before giving up."),
"innodb_log_buffer_size": z.int().min(1048576).max(4294967295).describe("The size in bytes of the buffer that InnoDB uses to write to the log files on disk."),
"innodb_online_alter_log_max_size": z.int().min(65536).max(1099511627776).describe("The upper limit in bytes on the size of the temporary log files used during online DDL operations for InnoDB tables."),
"innodb_read_io_threads": z.int().min(1).max(64).default(4).describe("The number of I/O threads for read operations in InnoDB. Changing this parameter will lead to a restart of the MySQL service."),
"innodb_rollback_on_timeout": z.boolean().describe("When enabled a transaction timeout causes InnoDB to abort and roll back the entire transaction. Changing this parameter will lead to a restart of the MySQL service."),
"innodb_thread_concurrency": z.int().min(0).max(1000).default(0).describe("Defines the maximum number of threads permitted inside of InnoDB. The default is `0` which indicates infinite concurrency, or no limit."),
"innodb_write_io_threads": z.int().min(1).max(64).default(4).describe("The number of I/O threads for write operations in InnoDB. Changing this parameter will lead to a restart of the MySQL service."),
"interactive_timeout": z.int().min(30).max(604800).describe("The number of seconds the server waits for activity on an interactive connection before closing it."),
"internal_tmp_mem_storage_engine": z.enum(["TempTable", "MEMORY"]).describe("The storage engine for in-memory internal temporary tables."),
"max_allowed_packet": z.int().min(102400).max(1073741824).default(7108864).describe("Size of the largest message in bytes that can be received by the server."),
"max_heap_table_size": z.int().min(1048576).max(1073741824).default(16777216).describe("Limits the size of internal in-memory tables. Also set `tmp_table_size`."),
"net_buffer_length": z.int().min(1024).max(1048576).default(16384).describe("Start sizes of connection buffer and result buffer. Changing this parameter will lead to a restart of the MySQL service."),
"net_read_timeout": z.int().min(1).max(3600).describe("The number of seconds to wait for more data from a connection before aborting the read."),
"net_write_timeout": z.int().min(1).max(3600).describe("The number of seconds to wait for a block to be written to a connection before aborting the write."),
"sql_mode": z.string().regex(/^[A-Z_]*(,[A-Z_]+)*$/).describe("Global SQL mode. Set to empty to use MySQL server defaults. When creating a new service and not setting this field, Akamai defaults to SQL mode which is strict, SQL standard compliant."),
"sql_require_primary_key": z.boolean().describe("Require primary key to be defined for new tables or old tables modified with `ALTER TABLE` and fail if missing. You should always have primary keys because various functionality may break if any large table is missing them."),
"tmp_table_size": z.int().min(1048576).max(1073741824).default(16777216).describe("Limits the size of internal in-memory tables. This also requires a `max_heap_table_size`."),
"wait_timeout": z.int().min(1).max(2147483).describe("The number of seconds the server waits for activity on a non-interactive connection before closing it.")
...
"subnet_id": z.int().describe("The unique identifier of a specific subnet, in the VPC (`vpc_id`) where the Managed Database can be accessed."),
"vpc_id": z.int().describe("The unique identifier of the VPC where this database exists.")
    }).describe("__Beta__ Restricts access to a MySQL Managed Database using a Virtual Private Cloud (VPC). Displayed as `null` if no VPC is configured.")),
...
"ssl_connection": z.boolean().default(true).describe("Currently required to be `true`. Whether to require SSL credentials to establish a connection to the Managed Database. Run the [Get managed MySQL database credentials](https://techdocs.akamai.com/linode-api/reference/get-databases-mysql-instance-credentials) operation for access information."),
 ...
 */
export const postDatabasesMysqlInstancesErrorSchema = z.object({
...
export const postDatabasesMysqlInstancesMutationRequestSchema = z.object({
...
"fork": z.optional(z.object({
    "restore_time": z.optional(z.string().datetime().describe("A specific database timestamp to restore from.")),
"source": z.int().describe("The unique instance id for the database to fork from. Run the [List Managed Databases](https://techdocs.akamai.com/linode-api/reference/get-databases-instances) operation and store the unique `id` for the target Managed Database.")
    }).describe("Include this object to restore a Managed Database by forking from a backup.\n\n- If you include this object, all other fields are optional.\n\n- Don't include this object if you're creating a new Managed Database.")),
"label": z.string().min(3).max(32).describe("__Filterable__ A unique, user-defined string referring to the Managed Database. This string needs to be unique per Managed Database engine type."),
...
"subnet_id": z.optional(z.int().describe("If the `vpc_id` includes multiple subnets, specify the `subnet_id` you want to use to control access to the database. Use the [List VPCs](https://techdocs.akamai.com/linode-api/reference/get-vpcs) operation to find and store the `id` of the relevant subnet object.\n\n> 📘\n>\n> A VPC needs at least one subnet to assign a Manage Database instance.")),
"vpc_id": z.optional(z.int().describe("The unique identifier of the VPC you want to use to enable private access to the Managed Database. Run the [List VPCs](https://techdocs.akamai.com/linode-api/reference/get-vpcs) operation and store the `id` for the applicable VPC."))
    }).describe("__Beta__ Restricts access on a MySQL Managed Database to a specific Virtual Private Cloud (VPC) configured for the cluster.").nullish(),
"region": z.string().describe("__Filterable__ The unique identifier for the [region](https://techdocs.akamai.com/linode-api/reference/get-regions) where the Managed Database lives."),
...
"type": z.string().describe("__Filterable__ The Linode Instance type used by the Managed Database for its nodes.")
    }).describe("Managed MySQL Database request object.")

export const postDatabasesMysqlInstancesMutationResponseSchema = z.lazy(() => postDatabasesMysqlInstances200Schema)