/**
* Generated by Kubb (https://kubb.dev/).
* Do not edit manually.
*/


export const postCreditCardPathParamsApiVersionEnum = {
    "v4": "v4",
    "v4beta": "v4beta"
} as const;

export type PostCreditCardPathParamsApiVersionEnumKey = (typeof postCreditCardPathParamsApiVersionEnum)[keyof typeof postCreditCardPathParamsApiVersionEnum];

export interface PostCreditCardPathParams {
    /**
     * @description __Enum__ Call either the `v4` URL, or `v4beta` for operations still in Beta.
     * @type string
    */
    apiVersion: PostCreditCardPathParamsApiVersionEnumKey;
}

/**
 * @description Credit Card updated.
*/
export type PostCreditCard200 = object;

/**
 * @description See [Errors](https://techdocs.akamai.com/linode-api/reference/errors) for the range of possible error response codes.
*/
export interface PostCreditCardError {
    /**
     * @type array | undefined
    */
    errors?: {
        /**
         * @description The field in the request that caused this error. This may be a path, separated by periods in the case of nested fields. In some cases this may come back as `null` if the error is not specific to any single element of the request.
         * @type string | undefined
        */
        field?: string;
        /**
         * @description What happened to cause this error. In most cases, this can be fixed immediately by changing the data you sent in the request, but in some cases you will be instructed to [Open a support ticket](https://techdocs.akamai.com/linode-api/reference/post-ticket) or perform some other action before you can complete the request successfully.
         * @type string | undefined
        */
        reason?: string;
    }[];
}

/**
 * @description Update the credit card information associated with your Account.
*/
export interface PostCreditCardMutationRequest {
    /**
     * @description Your credit card number. No spaces or hyphens (`-`) allowed.
     * @minLength 14
     * @maxLength 24
     * @type string, digits
    */
    card_number: string;
    /**
     * @description CVV (Card Verification Value) of the credit card, typically found on the back of the card.
     * @minLength 3
     * @maxLength 4
     * @type string, digits
    */
    cvv: string;
    /**
     * @description A value from 1-12 representing the expiration month of your credit card.\n\n  - 1 = January\n  - 2 = February\n  - 3 = March\n  - Etc.
     * @minLength 1
     * @maxLength 12
     * @type integer
    */
    expiry_month: number;
    /**
     * @description A four-digit integer representing the expiration year of your credit card.\n\nThe combination of `expiry_month` and `expiry_year` must result in a month/year combination of the current month or in the future. An expiration date set in the past is invalid.
     * @minLength 4
     * @maxLength 4
     * @type integer
    */
    expiry_year: number;
}

export type PostCreditCardMutationResponse = PostCreditCard200;

export type PostCreditCardMutation = {
    Response: PostCreditCard200;
    Request: PostCreditCardMutationRequest;
    PathParams: PostCreditCardPathParams;
    Errors: any;
};