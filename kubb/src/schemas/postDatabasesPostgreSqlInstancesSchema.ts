...
export const postDatabasesPostgreSqlInstancesPathParamsSchema = z.object({
 ...
 */
export const postDatabasesPostgreSqlInstances200Schema = z.object({
...
"hosts": z.optional(z.object({
    "primary": z.string().describe("The primary host for the Managed Database.").nullish(),
"secondary": z.string().describe("The secondary/private network host for the Managed Database. A private network host and a private IP can only be used to access a database cluster from Linodes in the same data center and will not incur transfer costs.\n\n> 📘\n>\n> The secondary hostname is publicly viewable and accessible.").nullish()
 ...
 */
export const postDatabasesPostgreSqlInstancesErrorSchema = z.object({
...
export const postDatabasesPostgreSqlInstancesMutationRequestSchema = z.object({
    "allow_list": z.optional(z.array(z.string().regex(/^([0-9]{1,3}\.){3}[0-9]{1,3}(\/([0-9]|[1-2][0-9]|3[0-2]))?$/)).min(0).describe("Controls access to the Managed Database.\n\n- Individually included IP addresses or CIDR ranges can access the Managed Database while all other sources are blocked.\n\n- A standalone value of `0.0.0.0/0` allows all IP addresses access to the Managed Database.\n\n- An empty array (`[]`) blocks all public and private connections to the Managed Database.")),
"cluster_size": z.optional(z.union([z.literal(1), z.literal(2), z.literal(3)]).default(1).describe("The number of Linode instance nodes deployed to the Managed Database.\n\n - Choose `3` nodes to create a high availability cluster that consists of one primary node and two replica nodes.\n\n- A `2` node cluster is only available with a dedicated plan. It consists of one primary node and one replica node.")),
...
"ssl_connection": z.optional(z.boolean().default(true).describe("Currently required to be `true`. Whether to require SSL credentials to establish a connection to the Managed Database. Run the [Get managed PostgreSQL database credentials](https://techdocs.akamai.com/linode-api/reference/get-databases-postgre-sql-instance-credentials) operation for access information.")),
"type": z.string().describe("__Filterable__ The Linode Instance type used by the Managed Database for its nodes.")
    }).describe("Managed PostgreSQL Database request object.")

export const postDatabasesPostgreSqlInstancesMutationResponseSchema = z.lazy(() => postDatabasesPostgreSqlInstances200Schema)