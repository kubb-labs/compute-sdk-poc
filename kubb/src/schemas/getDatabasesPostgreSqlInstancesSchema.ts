...
export const getDatabasesPostgreSqlInstancesPathParamsSchema = z.object({
    "apiVersion": z.enum(["v4", "v4beta"]).describe("__Enum__ Call either the `v4` URL, or `v4beta` for operations still in Beta.")
    })

export const getDatabasesPostgreSqlInstancesQueryParamsSchema = z.object({
 ...
 */
export const getDatabasesPostgreSqlInstances200Schema = z.object({
...
"subnet_id": z.optional(z.int().describe("The specific subnet, in the VPC (`vpc_id`) where the Managed Database cluster can be accessed.")),
"vpc_id": z.optional(z.int().describe("The unique identifier of the VPC where this database exists."))
    }).describe("__Beta__ Restricts access to a PostgreSQL Managed Database using a Virtual Private Cloud (VPC). Displayed as `null` if no VPC is configured.").nullish(),
 ...
 */
export const getDatabasesPostgreSqlInstancesErrorSchema = z.object({
...
export const getDatabasesPostgreSqlInstancesQueryResponseSchema = z.lazy(() => getDatabasesPostgreSqlInstances200Schema)