...
export const postSharegroupImagesPathParamsSchema = z.object({
 ...
 */
export const postSharegroupImages200Schema = z.object({
    "capabilities": z.array(z.string()).describe("__Read-only__ A list of the possible capabilities of this image."),
"created": z.string().datetime().describe("__Read-only__ When this image was shared."),
...
"tags": z.array(z.string().min(3).max(100)).min(0).max(500).describe("__Read-only__ The shared image's organizational tags. A tag can be from 3 to 100 characters long, and each image can include up to 500 tags."),
"total_size": z.int().describe("__Read-only__ The total size in bytes of all instances of this image, in all `regions`."),
"type": z.enum(["manual", "automatic", "shared"]).describe("__Read-only__ Indicates how the image was created. It's always `shared` for images in a shared group. An `automatic` image is created automatically from a deleted compute instance. Other users within share groups can access a `shared` image."),
"updated": z.nullable(z.string().datetime().describe("__Read-only__ When this shared image was last updated.")),
 ...
 */
export const postSharegroupImagesErrorSchema = z.object({
...
export const postSharegroupImagesMutationRequestSchema = z.object({
...
export const postSharegroupImagesMutationResponseSchema = z.lazy(() => postSharegroupImages200Schema)