...
export const putImagePathParamsSchema = z.object({
 ...
 */
export const putImage200Schema = z.object({
 ...
 */
export const putImageErrorSchema = z.object({
...
export const putImageMutationRequestSchema = z.object({
    "capabilities": z.optional(z.array(z.string()).describe("__Read-only__ A list of the possible capabilities of this image.\n\n- `cloud-init`. The image supports the cloud-init multi-distribution method with our [Metadata service](https://www.linode.com/docs/products/compute/compute-instances/guides/metadata/#troubleshoot-metadata-and-cloud-init). This only applies to public images.\n\n- `distributed-sites`. Whether the image can be used in distributed compute regions. Compared to a core compute region, distributed compute regions offer limited functionality, but they're globally distributed. Your image can be geographically closer to you, potentially letting you deploy it quicker. See [Regions and images](https://techdocs.akamai.com/cloud-computing/docs/images#regions-and-images) for complete details.")),
"created": z.optional(z.string().datetime().describe("__Read-only__ When this image was created.")),
"created_by": z.optional(z.string().describe("__Read-only__ The name of the user who created this image, or `linode` for public images.")),
"deprecated": z.optional(z.boolean().describe("__Filterable__, __Read-only__ A `true` value indicates a deprecated image. Only public images can be deprecated.")),
"description": z.string().min(1).max(65000).describe("A detailed description of this image.").nullish(),
"eol": z.string().datetime().describe("__Read-only__ The time of the public image's planned removal from service. This is `null` for private images.").nullish(),
"expiry": z.string().datetime().describe("__Read-only__ Only images created automatically from a deleted compute instance (type=automatic) expire. This is `null` for private images.").nullish(),
"id": z.optional(z.string().describe("__Read-only__ The unique identifier for each image.")),
"is_public": z.optional(z.boolean().describe("__Filterable__, __Read-only__ A `true` value if the image is a public distribution image. A `false` value indicates private, account-specific images.")),
"is_shared": z.optional(z.enum(["true", "false", "none"]).describe("__Filterable__, __Read-only__ A `true` value for shared private images. `none` for images shared within a group.")),
"label": z.optional(z.string().describe("__Filterable__ A short description of the image.")),
"regions": z.optional(z.array(z.object({
    "region": z.optional(z.string().describe("The unique identifier for the core compute region where this image is stored.")),
"status": z.optional(z.enum(["available", "creating", "pending", "pending deletion", "pending replication", "replicating"]).describe("The status of the image in this `region`. Possible values are `available`, `creating`, `pending`, `pending deletion`, `pending replication`, or `replicating`."))
    })).describe("__Read-only__ Details on the regions where this image is stored. See [Regions and images](https://techdocs.akamai.com/cloud-computing/docs/images#regions-and-images) for full details on support for `regions`.")),
"size": z.optional(z.int().describe("__Filterable__, __Read-only__ The minimum size in MB this image needs to deploy.")),
"status": z.optional(z.enum(["creating", "pending_upload", "available"]).describe("__Filterable__, __Read-only__ The current status of the image. Possible values are `available`, `creating`, and `pending_upload`.\n\n> 📘\n>\n> The `+order_by` and `+order` operators are not available when [filtering](https://techdocs.akamai.com/linode-api/reference/filtering-and-sorting) on this key.")),
"tags": z.optional(z.array(z.string().min(3).max(100)).min(0).max(500).describe("__Filterable__ Tags used for organizational purposes. A tag can be from 3 to 100 characters long, and an image can have a maximum of 500 total tags.")),
"total_size": z.optional(z.int().describe("__Read-only__ The total size in bytes of all instances of this image, in all `regions`.\n\n> 📘\n>\n> This object is empty for existing images. It's intended for use with future functionality.")),
"type": z.optional(z.enum(["manual", "automatic"]).describe("__Filterable__, __Read-only__ How the image was created. Create a `manual` image at any time. An `automatic` image is created automatically from a deleted compute instance.")),
"updated": z.optional(z.string().datetime().describe("__Read-only__ When this image was last updated.")),
"vendor": z.string().describe("__Filterable__, __Read-only__ The upstream distribution vendor. This is `null` for private images.").nullish()
    }).omit({ 'capabilities': true,'created': true,'created_by': true,'deprecated': true,'eol': true,'expiry': true,'id': true,'is_public': true,'is_shared': true,'regions': true,'size': true,'status': true,'total_size': true,'type': true,'updated': true,'vendor': true })

export const putImageMutationResponseSchema = z.lazy(() => putImage200Schema)